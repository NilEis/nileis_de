...
export type Apod = {
  copyright?: string; date: string; explanation: string; hdurl: string;
  media_type: string;
...
const year_2025: Apod[] = import.meta.env.NASA_API !== undefined ?
    (await (await fetch(`https://api.nasa.gov/planetary/apod?api_key=${
                import.meta.env.NASA_API}&start_date=2025-01-01`))
         .json()) as Apod[] :
    [];

export const apodData: Apod[] = shuffle([
  ...(year_1999 as Apod[]), ...(year_2000 as Apod[]), ...(year_2001 as Apod[]),
  ...(year_2002 as Apod[]), ...(year_2003 as Apod[]), ...(year_2004 as Apod[]),
  ...(year_2005 as Apod[]), ...(year_2006 as Apod[]), ...(year_2007 as Apod[]),
  ...(year_2008 as Apod[]), ...(year_2009 as Apod[]), ...(year_2010 as Apod[]),
  ...(year_2011 as Apod[]), ...(year_2012 as Apod[]), ...(year_2013 as Apod[]),
  ...(year_2014 as Apod[]), ...(year_2015 as Apod[]), ...(year_2016 as Apod[]),
  ...(year_2017 as Apod[]), ...(year_2018 as Apod[]), ...(year_2019 as Apod[]),
  ...(year_2020 as Apod[]), ...(year_2021 as Apod[]), ...(year_2022 as Apod[]),
  ...(year_2023 as Apod[]), ...(year_2024 as Apod[]), ...(year_2025)
].filter((apod) => apod.media_type !== 'video'));

const rle_encoded_as_string =
    JSON.stringify(encodeApod(JSON.stringify(apodData)));

export const GET: APIRoute =
    () => {
      return new Response(
          rle_encoded_as_string,
